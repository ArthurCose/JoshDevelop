...
const LOGIN_PAGE = path.resolve("web/views/login.html");
...
class Authentication
{
  ...
  static async register(username, password, verifyPassword)
  {
    ...
    await fs.mkdir(User.USERS_FOLDER).catch((err) => {
      if(err.code != "EEXIST")
        console.log(`Error creating USERS_FOLDER for registration: ${err.code}`);
    });
